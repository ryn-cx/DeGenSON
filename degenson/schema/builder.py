...
class _MetaSchemaBuilder(type):
    def __init__(cls, name, bases, attrs):
        ...
        cls.NODE_CLASS = type(
            "%sSchemaNode" % name, (SchemaNode,), {"STRATEGIES": cls.STRATEGIES}
        )
...
class SchemaBuilder(metaclass=_MetaSchemaBuilder):
    ...
    def __init__(self, schema_uri="DEFAULT"):
        ...
        if not issubclass(self.NODE_CLASS, SchemaNode):
            raise TypeError(
                "NODE_CLASS %r is not a subclass of SchemaNode" % self.NODE_CLASS
            )
    ...
    def __eq__(self, other):
        ...
        return (
            self._base_schema() == other._base_schema()
...
class Schema(SchemaBuilder):
    ...
    def to_dict(self, recurse="DEPRECATED"):
        warn(
            "#to_dict is deprecated in v1.0, and it may be removed in "
            "future versions. Use #to_schema instead.",
            PendingDeprecationWarning,
        )
        if recurse != "DEPRECATED":
            warn(
                "the `recurse` option for #to_dict does nothing in v1.0",
                DeprecationWarning,
            )
