...
class TestBasic(unittest.TestCase):
    ...
    def test_empty_object_stdin(self):
        (stdout, stderr) = run(stdin_data="{}")
        self.assertEqual(stderr, "")
        self.assertEqual(json.loads(stdout), dict({"type": "object"}, **BASE_SCHEMA))
...
class TestError(unittest.TestCase):
    ...
    BAD_JSON_MESSAGE = stderr_message(
        "invalid JSON in %s: Expecting value: line 1 column 1 (char 0)" % BAD_JSON_FILE
    )
    ...
    def assertEqualIgnoreWhitespace(
        ...
        return self.assertEqual(
            " ".join(first.split()), " ".join(second.split()), msg=msg
        )

    def test_no_input(self):
        (stdout, stderr) = run()
        self.assertEqualIgnoreWhitespace(
            stderr, stderr_message("noting to do - no schemas or objects given")
        )
...
class TestEncoding(unittest.TestCase):
    def test_encoding_unicode(self):
        (stdout, stderr) = run(["-e", "utf-8", fixture("utf-8.json")])
        self.assertEqual(stderr, "")
        self.assertEqual(json.loads(stdout), dict({"type": "string"}, **BASE_SCHEMA))

    def test_encoding_cp1252(self):
        (stdout, stderr) = run(["-e", "cp1252", fixture("cp1252.json")])
        self.assertEqual(stderr, "")
        self.assertEqual(json.loads(stdout), dict({"type": "string"}, **BASE_SCHEMA))
