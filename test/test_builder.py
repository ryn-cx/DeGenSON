...
class TestMethods(base.SchemaBuilderTestCase):
    def test_add_schema(self):
        ...
        self.assertResult({"$schema": SchemaBuilder.DEFAULT_URI, "type": "null"})
    ...
    def test_to_json(self):
        self.assertEqual(
            self.builder.to_json(), '{"$schema": "%s"}' % SchemaBuilder.DEFAULT_URI
        )
...
class TestInteraction(base.SchemaBuilderTestCase):
    def test_add_other(self):
        ...
        self.assertResult({"$schema": SCHEMA_URI, "type": ["integer", "string"]})
